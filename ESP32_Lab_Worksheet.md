...
1. **Memory Types**: SRAM และ Flash Memory ใช้เก็บข้อมูลประเภทไหน?
  - SRAM: ใช้เก็บข้อมูลที่เปลี่ยนแปลงได้ เช่น ตัวแปรใน stack, heap, global variables
  - Flash Memory: ใช้เก็บข้อมูลคงที่ เช่น string literals, code ที่ไม่เปลี่ยนแปลง

2. **Address Ranges**: ตัวแปรแต่ละประเภทอยู่ใน address range ไหน?
  - Stack (stack_var): 0x3ffb4550 → อยู่ใน SRAM
  - Global SRAM (sram_buffer): 0x3ffb16ac → อยู่ใน SRAM
  - Flash (flash_string): 0x3f407b48 → อยู่ใน Flash
  - Heap (heap_ptr): 0x3ffb5264 → อยู่ใน SRAM
3. **Memory Usage**: ESP32 มี memory ทั้งหมดเท่าไร และใช้ไปเท่าไร?
  - SRAM ทั้งหมด: 520,192 bytes → ใช้ไปแล้ว = 520,192 - 380,096 = 140,096 bytes
  - Flash: มี 2,097,152 bytes (ใช้ไปไม่ระบุ)
  - DMA Memory: มี 303,096 bytes (ใช้ไปไม่ระบุ)
---
...
1. **Cache Efficiency**: ทำไม sequential access เร็วกว่า random access?
  - Cache Efficiency Sequential access เร็วกว่าเพราะ CPU cache prefetch ได้ข้อมูลหลายตัวในคราวเดียว → ลดการเข้าถึงหน่วยความจำหลัก
2. **Memory Hierarchy**: ความแตกต่างระหว่าง internal SRAM และ external memory คืออะไร?
  - Internal SRAM: อยู่ในชิป → เร็ว, latency ต่ำ
  - External PSRAM: ต่อผ่าน SPI → ช้ากว่า, latency สูง
3. **Stride Patterns**: stride size ส่งผลต่อ performance อย่างไร?
  - Stride เล็ก (1) → เข้าถึงต่อเนื่อง → cache ทำงานดี → เร็ว
  - Stride ใหญ่ (8, 16) → ข้ามเยอะ → cache ใช้ไม่คุ้ม → ช้าลง
---
...
1. **Core Specialization**: จากผลการทดลอง core ไหนเหมาะกับงานประเภทใด?
  - Core 0 (63 μs) → เหมาะกับงานที่ต้องการความเร็ว เช่น ควบคุมอุปกรณ์, protocol
  - Core 1 (9694 μs) → เหมาะกับงานคำนวณหนัก เช่น ประมวลผลข้อมูล, background tasks
2. **Communication Overhead**: inter-core communication มี overhead เท่าไร?
  - Latency เฉลี่ย 12 ms ต่อข้อความ → มี overhead พอสมควร โดยเฉพาะถ้า Core 1 กำลังยุ่ง
3. **Load Balancing**: การกระจายงานระหว่าง cores มีประสิทธิภาพหรือไม่?
  - งานกระจายได้ดี → ทั้งสอง Core ทำงานเสร็จใกล้เคียงกัน → FreeRTOS จัดการ load ได้มีประสิทธิภาพ
---
